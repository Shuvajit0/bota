...
function PureModelSelectorCompact({
  ...
  const allowedModelIds = new Set<string>(
    modelsData?.entitlements?.allowedModelIds ?? chatModels.map((m) => m.id)
  );
            ...
            return sortedKeys.map((key) => (
              ...
              >
                {grouped[key].map(({ model, curated }) => {
                  ...
                  const isAllowed = allowedModelIds.has(model.id);
                  const isSelectable = curated && isAllowed;
                  return (
                    <ModelSelectorItem
                      className={cn(
                        "flex w-full",
